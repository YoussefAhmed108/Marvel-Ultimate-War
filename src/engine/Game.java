...
public class Game {
	...
	public void castAbility(Ability a)
		...
		if (a.getCastArea() == AreaOfEffect.SELFTARGET) {
			...
			for (Champion c : team) {
				...
				boolean flag1 = (a instanceof DamagingAbility) && (this.hasEffect(c, "Dodge"));
				boolean flag2 = (a instanceof CrowdControlAbility) && (((CrowdControlAbility) a).getEffect().getType() == EffectType.DEBUFF) && (this.hasEffect(c, "Dodge")) ;
				if(!(flag1 || flag2)) {
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		if (getCurrentChampion() instanceof Hero) {
			...
			if(this.firstPlayer.getTeam().contains(this.getCurrentChampion())) {
				for (Champion c : secondPlayer.getTeam()) {
